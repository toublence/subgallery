            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text(L10n.text("보관 기한, 보안, 동기화를 제한 없이 사용하세요.")).foregroundStyle(.secondary).multilineTextAlignment(.center)
                    }

                    VStack(alignment: .leading, spacing: 14) {
                        FeatureRow("Temporary 자동 정리", symbol: "clock.arrow.circlepath")
                        FeatureRow("PIN 잠금", symbol: "lock.fill")
                    ...
                    if purchases.products.isEmpty {
                        ContentUnavailableView(
                            L10n.text("상품을 불러올 수 없음"),
                            systemImage: "cart",
                            description: Text(L10n.text("App Store Connect에서 구독 상품을 설정해 주세요."))
                        )
                    } else {
                            ...
                            Button { selectedID = product.id } label: {
                                HStack {
                                    VStack(alignment: .leading) {
                                        ...
                                        if product.id.hasSuffix("yearly") { Text(L10n.text("추천")).font(.caption).foregroundStyle(.tint) }
                                    }
                    ...
                    Button(L10n.text("구매 복원")) { Task { await purchases.restore() } }.font(.footnote)
                    Text(L10n.text("구매 시 Apple의 이용 약관과 개인정보 처리방침이 적용됩니다."))
